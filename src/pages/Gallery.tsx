          ...
          <div className="flex flex-wrap justify-center gap-2 mb-10">
            {categories.map((c) => (
                <button
                  key={c}
                  onClick={() => setFilter(c)}
                  className={`px-5 py-2 rounded-full font-display text-sm tracking-wide animate-pulse-soft transition-colors ${
                    filter === c
                      ? "bg-primary text-primary-foreground shadow-glow"
                      : "bg-muted text-muted-foreground hover:bg-primary/10 hover:shadow-glow"
                  }`}
                >
                {c}
              </button>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {filtered.map((item) => (
              <button
                ...
                className={`aspect-square rounded-xl bg-gradient-to-br ${categoryGradients[item.category]} flex items-end p-4 hover:scale-[1.02] hover:shadow-float shadow-glow transition-all duration-300 animate-fade-in cursor-pointer`}
              >
